...
func TestPostTape(t *testing.T) {
	...
	err = json.Unmarshal(w.Body.Bytes(), &response)
	assert.NoError(t, err)
	assert.Equal(t, 6, len(handlers.Tapes))
	assert.Equal(t, 1, handlers.Tapes[5].Quantity)
	assert.Equal(t, "A torinói ló", handlers.Tapes[5].Title)

	// Test adding a tape which already exists in the tapes slice
	w2 := httptest.NewRecorder()
	request2 := handlers.Tape{
		ID:       "7",
		Title:    "A torinói ló",
		Director: "Tarr Béla",
		Genre:    "Drama",
	...
	assert.NoError(t, err2)
	assert.Equal(t, 6, len(handlers.Tapes))
	assert.Equal(t, 2, handlers.Tapes[5].Quantity)
	assert.Equal(t, fmt.Sprintf("The title, \"%s\", is already in the catalog. We now have %d units in stock.", handlers.Tapes[5].Title, handlers.Tapes[5].Quantity), response2["message"])
